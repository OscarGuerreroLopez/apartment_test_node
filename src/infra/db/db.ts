...
const favoriteCollection: IObjectLiteral[] = [];

export const DbAdapter = (): Database => {
  if (Array.from(dbInstancesModels.keys()).length === 0) {
    dbInstancesModels.set("user", fakeModel(userCollection));
    dbInstancesModels.set("apartment", fakeModel(apartmentCollection));
    dbInstancesModels.set("favorite", fakeModel(favoriteCollection));
  }
